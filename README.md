...
## 最近更新
- 新增：打包前自动清空项目根目录 `Logs` 文件夹（仅 Editor 生效）
- 新增：可选「历史日志归档」能力，自动把旧日志移动到 `Archive` 子目录
- 变更记录：详见 `Assets/package/CHANGELOG.md`
...
- 每次打包前自动清空 `Logs` 文件夹内容，避免旧日志混入新包流程
- 可选启用归档：当前日志以外的旧日志会自动移入 `Archive` 目录，避免主目录堆积
